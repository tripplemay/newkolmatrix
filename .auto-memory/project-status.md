...
## 已上线
- `https://newkol.guangai.ai` 跑 **M4.6 + M4.7 + M4.8 @ `c9236af1cc65cc64a0e7ecd50f15c86def0428d9`**（2026-08-04 部署，用户授权后由主实例触发；回滚点 = 上一版 M4+M4.5 `fd751ad5cd0e1de306a2f59c17344e7ba26e2be2`）
- 部署实况：无 compose/env/迁移变更的干净部署 · workflow success · 公网复核 `/api/health` ok、`/` 307→`/admin/today` 200、`/api/agent` GET 405（存活）。⚠️ image_tag 必须完整 40 位 SHA；compose 是 VPS 人工副本
- **上线后待人工 L2 复测**：M4.7 原始故障场景（环节页问跨域问题，前台应受理不拒答）+ 真模型自主选专家质量 + M4.8 超时告知真网关表现（清单见两批 signoff §7 / M4.7 §29）
