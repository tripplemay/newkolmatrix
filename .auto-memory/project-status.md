...
---
## 当前批次
- **M5.1b-TENANT-INJECTION，status=verifying（2026-08-08）**。7/8 完成，全部 `executor:generator` 条目已交付；
  仅剩 **F001（executor:evaluator）= 补验 M5.1 遗留实现**，归隔离 evaluator
...
- L1 实跑：tsc 0 · lint 0 warning · `npx vitest run` 148 files / 1869 tests 全绿 · `rls:e2e` 15 passed
...
- proposed-learnings **7 条待裁**（本批新增 2 条：vitest 动态 import mock 并发失效 · 并发用例「集合相等」是伪断言）
