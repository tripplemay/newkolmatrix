...
---
## 当前批次
- **M4.7-FRONTDESK done ✅（2026-08-02，快车道，fix_rounds=3）** — 单一前台 + 专家降为内部能力，复验轮三 **11/11 PASS 签收**（轮一 8P/3PA → 轮二 7P/4PA → 轮三 11P；三轮卡点**全在测试鉴别力与文档一致性，零产品代码缺陷**）。轮三经 26 条独立变异/探针复核：RV-4 加测**反方向**（把「咨询也抬预算」写进 loop → RED(3)）· RV-2 三项子要求 E3/E4/E5 各 RED（轮二均全绿）· F009 两层清态 BP-A/BP-B 双双 EXIT=1 · S-RV2-9 核到 CI 那一层（ci.yml 走 `npm run test:visual`，前置确已串上）。**签收前提：§19-7 三条待 Planner 在 done 阶段裁决**（S-RV2-5 规则冲突 / S-RV2-8 主 loop 超时零告知+零留痕是否开为下批 feature / S-M47-G3-5 落库口径）+ 把 S-RV3-1/S-RV3-5 兜底写进 backlog。报告 `docs/test-reports/M4.7-FRONTDESK-signoff.md`（§21-§29 为轮三，新增 S-RV3-1…6）
...
- **M4.7 新证**：**改文档 ≠ 那句话被守住**——as-built 陈述必须钉成**双向行为**（只钉「接力抬」不钉「咨询不抬」= 半个钉子；轮三加测反方向才补齐。注：轮二 handoff 称该变异「全量无一条会红」，轮三在 `2ad0946` 上实测**证伪**，仍有 1 条会红）· 清理登记表杜绝子集漂移，但**删掉整条登记时断言跟着消失**，须再压一层不从登记表派生的整表普查（两层失效模式不重叠）· **`next build` 后必须重启 serve-standalone**，否则旧进程 chunk 指纹对不上、页面不 hydrate，伪装成整套视觉基线翻红 · **收编测试钉必须核到 CI 那一层**（`ci.yml` 跑的是 `npm run test:visual` 还是 `npx playwright test`，决定前置串没串上）
