...
- **每写一条断言，先想清楚「什么改动应该让它红」，然后真的去改一次看它红不红**——不做这一步，断言只提供心理安慰。断言强度分级（源码正则 < 黑名单否定 < 正向精确匹配 < 行为级）见 `framework/patterns/audit-methodology.md` §7
- **「让复验点名的那条变异翻红」≠「这条断言有鉴别力」**：修完自问「我能不能在 5 分钟内想出第二条绕过」，想得出就还没修完
