...
- **本批 native 工具**：`search_kols`（internal，向量检索 seed KOL）、`get_kol_detail`（internal）、`send_outreach`（outbound，F009 闸门样例）。
...
- `src/components/copilot/canvas/canvas-registry.tsx`：`CANVAS_REGISTRY` 把**工具名 → 渲染组件**。`renderToolResult(toolName, output)` / `hasCanvasRenderer(toolName)`。
...
- `AgentPersona = { id, name, stage, duty, isolation, uiSyntax, tools[], systemPrompt }`。`systemPrompt` 由 `duty`（我做什么）+ `isolation`（否定式护栏，我不做什么）组合——否定式比肯定式更难被模型伪造（D13 升级版）。
...
- `src/lib/agent/orchestrator.ts` + client-safe 的 `stage-routing.ts`：
  - **环节路由** `routeToStage(projectId, stage)`：把「进入某项目的某环节」表达为目标 CopilotContext（`/admin/campaigns/{id}?stage=`）。指令语法 `enter:/pick:/env:`（`parseOrchestratorDirective`）。五环节 `brief/match/reach/delivery/insight` ↔ `STAGE_AGENT`。
