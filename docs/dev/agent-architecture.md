...
- `AgentPersona = { id, name, stage, duty, isolation, uiSyntax, tools[], knowledgeKinds?, systemPrompt }`。`systemPrompt` 由 `duty`（我做什么）+ `isolation`（否定式护栏，我不做什么）组合——否定式比肯定式更难被模型伪造（D13 升级版）。`knowledgeKinds`（M1-D F005）= ⑤层知识注入的 kind 子集（FR-8.4.8 映射：strategy 三类全量 / match=受众 / reach=卖点 / compliance=红线；未声明不注入）。
