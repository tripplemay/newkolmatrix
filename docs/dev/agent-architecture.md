...
- **native 工具（as-built，M2-A 校准）**：`search_kols`（internal，向量检索 seed KOL）、`get_kol_detail`（internal）、`send_outreach`（outbound，F009 闸门样例）、`compute_health`（internal，M1-B）、`match_plan`（internal，M2-A：现行轮组合 + PlanKol 摘要，输出携带 `type:'match_plan'` 供 canvas type 路由）、`evaluate_creator`（internal，M2-A：`computeMatchScore` 单人可解释评估，画像口径与候选生成同源）。`create_project`（internal，M2-C：项目创建+OperationLog 留痕，挂 orchestrator/strategy——UI POST /api/projects 同服务）。**M3-A F003/F006 扩容**：`send_outreach` 接真（单人语法，async buildHarm 从 DB 读 contactEmail 如实披露 + P3 无邮箱明示拒绝 + `ops/email` 真投递/mock 回落 + crmInfer 重算）、`draft_email`/`refine_email`（internal，gateway chat 起草/改写，语言随 KOL.language，草稿落 OutreachMessage(direction=draft)）、`commit_quote`（outbound，harm 三要素金额/交付物/对象；Quote proposed→committed + P8 budgetUsd 回填）。`buildHarm` 自 M3-A 起支持 async（DB 读真值，不信任模型转述）。**M3-B F005/F006/F007/F011 扩容**：`payout`（outbound，harm 三行收款方/金额+币种/依据；**buildHarm 与 execute 各重跑一次 `deliveryCheck`，ready=false 一律拒**——FR-8.2.4.2 无绕过入口的服务端实现；执行 = mock `EscrowPartner.release()`（P1 零真实资金动作）+ `Payout prepared→released` + Deal 推进 completed，同事务）、`distribute_keys`（outbound，harm 三要素领取方/数量/「一经发放不可回收」；执行 = mock `KeyDistributor` + `GameKey reserved→distributed`；库存不足明示拒绝不部分发放）、`track_delivery`/`check_deliverables`（internal 只读，后者输出**就是** `deliveryCheck` 产物，不内联重算）、`confirm_brief_goal`（internal，brief 目标确认落库，与 UI `PATCH /api/projects/[id]/goal` 共用 `setProjectGoal`）。delivery 人格 tools 由空数组填为四件。**M4-INSIGHT F005/F006/F008 扩容**：`compute_roi`（internal 只读，输出**就是** `roi.compute` + `attribution.gaps` 产物——不内联重算；分子缺 → roi=null + insufficient_evidence 诚实透传，目标值接线 `Project.goal.targetExposure`）、`draft_report`（internal，gateway chat 长文起草（`AIGCGATEWAY_REPORT_MODEL` 大模型路由插座）→ `WeeklyReport(adopted=false)` 落库；无凭据降级固定草案首行明示；采纳 = 独立 internal 服务置 adopted，幂等不改写 adoptedAt）、`create_share_link`（outbound **白名单第 6**，harm 三要素可见范围/有效期/「链接一经生成即暴露」红标；执行 = mock `ShareLinkService`（**零真实公开暴露**）+ `ShareLink` 落库（gateLogId + tokenHash，明文 token 仅 execute 响应现一次）+ irrev 同事务；幂等键 = PendingAction.id，重入不双建且 token=null 如实返回）。insight 人格 tools 由空数组填为三件。
...
- `AgentPersona = { id, name, stage, duty, isolation, uiSyntax, tools[], knowledgeKinds?, systemPrompt }`。`systemPrompt` 由 `duty`（我做什么）+ `isolation`（否定式护栏，我不做什么）+ `uiSyntax`（我怎么呈现——M2-A F007 注入「你的产出形态：{uiSyntax}」段，:1032 欠账消解）组合——否定式比肯定式更难被模型伪造（D13 升级版）；M2-C 再加两段：**行动承诺诚实条款**（BASE_SYSTEM 产品级：工具真实返回成功才可声称已执行/超能力请求明说+指路/建议禁包装为已编排——触发源 = 用户实证幻觉编排事故）与**编队名册段**（PERSONA_SEED 同源生成 7 人格名单 + 禁杜撰名册外角色）；各段与 `personaBoundary` UI 卡同源不漂移。`knowledgeKinds`（M1-D F005）= ⑤层知识注入的 kind 子集（FR-8.4.8 映射：strategy 三类全量 / match=受众 / reach=卖点 / compliance=红线；未声明不注入）。match 人格 tools 已扩四件（M2-A：+= `match_plan`/`evaluate_creator`）。
...
| AI→人闸门服务端强制 + harm zod schema + 留痕同事务 | ~~报价 / 放款等更多 outbound 工具~~（✅ M3-A commit_quote / M3-B payout·distribute_keys 已兑现）、真实 partner 集成（M5） |
...
- **两步票据（M3-A F002，§9.3.2 全量）**：`confirmPendingAction`（`POST /api/actions/[id]/confirm`）= 签票——原子条件 UPDATE（WHERE status='pending'，并发败者 409）→ status=confirmed + 只存 ticketHash（**票明文仅在 confirm 响应出现一次**）；`executePendingAction`（`POST /api/actions/[id]/execute {ticket}`）= 消费票——原子条件 UPDATE（WHERE confirmed AND ticketUsedAt IS NULL AND ticketExpiresAt>now()）→ executing → 交互事务内执行（工具经 ctx.db 落库）→ **同一事务** executed + OperationLog kind:irrev + 业务态变更；失败 → failed、无 irrev 行。7 态：pending→confirmed→executing→executed/failed；pending→rejected；pending/confirmed→expired（惰性翻转）。分码：403 GATE_TOKEN_INVALID / 404 / 409 GATE_ALREADY_DECIDED / 410 GATE_EXPIRED；`/api/actions/*` 30/min/IP fail-open 限流（P9）。外部真投递以 `ctx.gateActionId`（=PA.id）为幂等键——crash 重放不双发。
...
- **拒绝**：`rejectPendingAction` → 真实 `rejected` 态（M3-A F002，清 expiresAt=epoch 债）+ `OperationLog kind:block`。
- **变异测试（D20 硬性，`scripts/test/gate-smoke.ts`）**：把服务端拦截退回原状（直调 `tool.execute` 绕过 `executeTool` 门控）→ 副作用发生 → G1「无副作用」断言必变红。证断言验**行为**、非验源码关键字。
- 验证：`npm run gate:smoke`（G1–G8：两步票据 + 7 态 + 分码负例 + 并发双确认/双消费竞态恰一胜 + D20 变异）；`npm run reach:e2e`（PRD 15.3 E2E：起草→审阅→点确认才发送，P1 仅测试邮箱）。
