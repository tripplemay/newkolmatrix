...
>
> **v1.2 校准（M1-C 修订）**：本章 §5.2–§5.5 除已标注者外仍以演进目标为主——当前 `prisma/schema.prisma` 8 个模型（§7.2.1 为实装权威）。**已实装例外**：§5.3① 环节游标（M1-A/M1-B）与 §5.4 的 health.compute / envGuards（M1-A）。概念层变更需显式修订本文（R13）。
...
**① 项目环节游标 `cur`**（责任链，FR-7.9；可回看已解锁、不可跳未解锁）— **已实装（M1-A F005/F006 + M1-B F004）**：`Project.cur/maxReached` 双值列 + `domain/env-guards.ts`（canEnter/canAdvance）+ `domain/env-advance.ts`（唯一写入点）+ 前端 selectEnv 拦截。as-built 注（M2-A F004 校准）：下表五条前置中 →brief（无条件）、→match（goal 判据）、**→reach（`hasApprovedMatchPlan` 真判定，`MATCH_PLAN_NOT_APPROVED`，EnvGuardContext 由两调用点组装）**均已真判；→delivery/insight 依赖表未建（M3），按 D9 保守拒 `DEPENDENCY_NOT_IMPLEMENTED`
...
### 5.4 领域服务（`src/lib/domain/`，纯函数注册表）— 部分实装（M3-A 校准：health.compute / envGuards / matchScore.compute / **crmInfer（`domain/crm-infer.ts`）** 已实装；deliveryCheck/roi/attribution 仍为演进目标归 M3-B+）
...
> **M1-C 校准**：`Project` 已有 `cur`/`maxReached`/`goal`/`budgetTotal`/`currency`/`market`/`gameId` 列（M1-A F003 expand 迁移）；`PendingAction` 已有 `projectId`/`agentId` 软引用（M1-C F002）。仍缺：`Game` **无 `Material`/`GameKnowledge` 关联表**（knowledge 域归 M1-D）。详情/列表/今天页已 RSC 直读（§6.7 校准），env-\* 语法面仍走 mock 契约层。
...
>
> 后果与承接方式：
> - 「谁/何时/做了什么/哪一类」由 `actor` + `createdAt` + `summary` + `kind` 承载；结构化字段级筛选（FR-11.13 的 `toolClass`/`reversible`/`projectId` 过滤）— `projectId`/`payloadJson` 列已落（M1-A D13 expand 迁移），筛选查询面未建（M1-D+）；后续扩展仍以加列迁移，不改已有行。
> - `kind` 四态语义（as-built 写入点）：`auto`=工具直接执行的可逆动作与系统事件（含 MockEmailSender 的 `SENT_MARKER` 行、CRM 状态推进/信号接入/人工覆盖留痕——M3-A）· `gate`=outbound 被闸门拦截落 pending · `block`=人拒绝 · `irrev`=人确认后不可逆执行完成。
...
**目标态 outbound 六工具白名单** = **部分实装**：`send_outreach`（✅）· `commit_quote`（✅ M3-A F006）· `send_bulk_outreach`（M3-B+）· `payout` · `distribute_keys`（M3-B）· `create_share_link`（M4）。
...
|---|---|---|---|---|---|
| `orchestrator` | 编排 Agent | 工作区层 | `[create_project]`（M2-C） | 不亲自执行环节工作，只分派与汇总 | 今天/雷达 |
| `strategy` | 策略 Agent | ① Brief | `[get_kol_detail, compute_health, create_project]`（M1-B / M2-C 扩） | 不联系创作者、不放款——交给触达/交付 | 仪表 |
...
### 9.8 副作用适配器（`src/lib/ops/`）— 部分实装（email ✅ M3-A F003；escrow/keys/share 归 M3-B+）
...
### 10.3 出站集成（ops 适配器）— 部分实装（email ✅ M3-A F003；其余归 M3-B+）
...
| M3-B（DELIVERY） | delivery 域 | Deal/Deliverable/GameKey/Payout + deliveryCheck + partner 适配器 + 资金状态机 + payout 闸门 + V7 台账接真 | 计划 |
...
| ADR-17 | 副作用走 ops 适配器接口，mock 先行、真集成后置 | §9.8 | 锁定 ✅ 已兑现首例（M3-A F003：`ops/email` 双实现，SENT_MARKER mock 转正为接口实现） |
