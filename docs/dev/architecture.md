...
**⑧ `MatchCandidate.verdict` — 已实装（M2-A F001/F003，D20 变异断言同上）**：`pending → kept / dropped`（人工裁定，写入口归 P8 后续批）；刷新 upsert **不回退**已 kept/dropped 的人工态（P4，update 分支不含 verdict）；dropped 出局组合生成。

### 5.4 领域服务（`src/lib/domain/`，纯函数注册表）— 部分实装（M2-A 校准：health.compute / envGuards / **matchScore.compute（`domain/match-score.ts`）** 已实装；crmInfer/deliveryCheck/roi/attribution 仍为演进目标归 M3+）
...
| M2-B（MATCH 深字段） | kol 深字段消费 | 创作者抽屉七分区接真 + Kol 深字段数据源裁决 + ProvenanceTag 接 Kol 真数据 + credibility/brandSafety zod refine + 价格数据（budgetUsd 真源另归 M3 CRM） | 计划 |
