...
**① 项目环节游标 `cur`**（责任链，FR-7.9；可回看已解锁、不可跳未解锁）— **已实装（M1-A F005/F006 + M1-B F004）**：`Project.cur/maxReached` 双值列 + `domain/env-guards.ts`（canEnter/canAdvance）+ `domain/env-advance.ts`（唯一写入点）+ 前端 selectEnv 拦截。as-built 注：下表五条前置中仅 →brief（无条件）与 →match（goal 判据）可判，→reach/delivery/insight 依赖表未建（M2/M3），按 D9 保守拒 `DEPENDENCY_NOT_IMPLEMENTED`
...
**⑤ `Material.parseStatus` — 演进目标（未实装，归 M1-D）**：`pending → parsing → parsed / failed`；failed 可重试回到 parsing。

**⑥ `GameKnowledge` 版本链 — 演进目标（未实装，归 M1-D）**：新解析生成新条目，旧条目 `supersededById` 指向新条目（不物理删除，FR-11.11）；读取恒取链头。

### 5.4 领域服务（`src/lib/domain/`，纯函数注册表）— 部分实装（M1-C 校准：health.compute → `domain/health.ts` + envGuards → `domain/env-guards.ts` 已落 M1-A；matchScore/crmInfer/deliveryCheck/roi/attribution 仍为演进目标归 M2+）
...
### 7.6 游戏知识平面（平面二）— 演进目标（未实装，归 M1-D）
...
### 8.10 主动式 Agent（例程调度）— 骨架已实装（M1-C F004：`lib/jobs/` scheduler + 互斥锁 + health-scan；其余 4 条规划例程随 M2-M4 表落地）
...
| M1-D（KNOWLEDGE） | knowledge 域（原 M1 清单拆出） | Material/GameKnowledge 表 + 上传通道 + 知识解析管道（同步 + 轮询，ADR-19）+ 页面接真 + 存储后端裁决 | 计划 |
