...
> `bd.scope` = 东南亚 **+ 日韩**（`gsea` / `hsr` / `zzz` / `new`）。`sky`（星穹·老带新，已进 06 复盘）不在其内 —— 保留一个 scope 外项目，对象级权限才有可演示、可回归的对象。

### 3.1.5 ⚠️ 三角色体系已废止（2026-07-16 用户裁决）

**产品定位 = 服务【中小团队】的 AI 驱动营销工具。中小团队没有「营销负责人 / KOL BD / 财务」三张桌子 —— 常常一两个人从头干到尾。**

**根源诊断（错在实现方**）：角色体系是从另一个产品（`有墨公考 · AI业务工作台`）借来的，不是从本产品定位推导出来的。借鉴的前提是两个产品的用户结构相似 —— 这一点从未被验证。
**更根本的矛盾**：本项目的原始需求第一句就是「避免旧系统过于像传统 SaaS 的实现方式，重构为 AI-native」，而**「提交 → 审批 → 退回 → 重提」恰恰是传统 SaaS 最典型的特征**。引入角色体系是往需求的反方向走了一程。

**下列决策全部作废：** D11（环节 owner）· D12（交接链含退回）· D14（闸门阈值量化）· D18（审批者≠发起者）· D23（驾驶舱按角色作业单元）· D24（财务驾驶舱=资金工作台）。
**D13 部分保留**：「否定式护栏」保留并升级 —— 从「角色的数据边界」改为「**AI 的行为边界**」；「copilotScope 真拒答」作废（没有跨角色边界可拒）。
**features.json F008 整条作废**，F001 的 `User.role/scope` 与 `Approval` 表、F004 的 `allowedRoles` gating、F006 的角色切换器一并移除。

### 3.1.6 单角色架构（D26–D29）

| # | 决策 | 依据 |
|---|---|---|
| **D26** | **单角色推进全流程。** 无 role/scope/权限四层/审批链/角色切换器 | 中小团队定位。权限体系服务的是「组织分工」，我们的用户没有分工可分。 |
| **D27** | **删【人→人】审批，留【AI→人】闸门。** 凡对外、不可逆、花钱的动作（发信 / 批量发 / 报价 / 分发 key / 放款 / 对外分享），AI 只能走到「已备好，等你按」 | 两者形式相似（都要点确认）、**本质相反**：前者是组织层级的产物（传统 SaaS），后者是 AI-native 的信任边界。**拆掉角色后闸门反而更纯粹** —— 文案从「提交营销负责人审批」变成「这是对外不可撤销的，你确认吗」。<br>**推论**：内部可改、可撤销、不花钱的动作（选组合、推翻规则、复核审核项、采纳复盘）**不该有闸门** —— 给它们加确认框只会训练用户无脑点确认，反而稀释真闸门的分量。 |
| **D28** | **删阈值（$8,000 / $2,000 / 10 封）。** 所有对外动作一律一次确认 | 用户决策（实现方建议改为「确认的重量」被否）。阈值原本的语义是「超额度给上级批」，没有上级了它就没有意义。<br>**但闸门必须如实说明利害**：批量确认框列出全部收件人、报价框标金额与不可撤销。**那不是阈值分级，那只是不撒谎。** |
| **D29** | **保留「负责人」标记（`owner: 你/Sarah/Kenji`）—— 是分工不是权限** | 中小团队 2-3 人同样权限，但要知道「这个谁在跟」，免得两人同时给一位创作者发信。 |

---

### 3.2 第二轮独立复审后追加的决策（D16–D19）

> ⚠️ **D18 已随三角色体系作废**（见 §3.1.5）。D16/D17 的「组合生效」语义保留，但 `direct/approved` 之分作废 —— 单角色下**选了就生效**。

来源：`docs/audits/KOLMatrix-interaction-prototype-reaudit-2026-07-15.md`（判 FAIL，P0×3，探针逐条核实全部属实）。

| # | 决策 | 依据（血的教训） |
|---|---|---|
| **D16** | **组合生效是按项目的业务态 `ACTIVE_PLAN{cid:{id,via}}`，不是全局 `selectedPlan`；`direct` 与 `approved` 都算生效** | 原 `effectivePlan()` 只认 approved 审批单 → **Lead 是组合 owner，自己拍板反而不生效，Reach 起草 0 封**。owner 的核心路径整条不可用，而 E2E 恰好只测了「BD 超阈值→Lead 批」那条偶然可走的路，**测试绕过了 bug**。 |
| **D17** | **阈值内直接生效**：BD 选 ≤ `$8,000` 的组合立即生效，超出才转审批；Lead（owner）恒直接生效 | 用户决策。若所有组合一律须批，`THRESHOLD.planBudget = $8,000` 就没有意义 —— **定了阈值却不让它决定任何事，等于没定**。 |
| **D18** | **审批者 ≠ 发起者**：持 `approve-X` 不等于可以发起 `X`。对外承诺类动作只认 `submit-X` | 原 terms 守卫写成 `(!submit-quote && !approve-quote)`，Lead 持 `approve-quote` 即放行 → **审批者直接拿到「确认报价并回信」= 自己发起自己批准**，责任链短路。这个洞在 Lead 路径修好之前是「够不到」的，所以一直没暴露。 |
| **D19** | **资金按生命周期分环节**：04 谈定条款 → 签署托管；**05 交付审核通过 → 放款**。合规审核项必须有对应的人工复核动作 | 原结算面板钉在 04，而 `payoutReady` 要求 `stage >= 5` → **用户被迫回到历史环节找放款按钮，且实际没有任何一条走得通的资金路径**（gsea 的 `#ad` 固定为 0 且无动作可改；hsr 的 `#ad` 已过却不在 BD scope 内）。**一个永远无法满足的前置条件 = 一个假控件。** |

### 3.3 第二次上手校准后的 IA 反转（D21–D24）

来源：用户实测原型后的三条反馈 —— ①角色定位与职能分工没界定、工作流交叉 ②环节工作台按跨项目设计、心流无法聚焦单项目 ③环节工作台趋同、没按角色在本环节的职责设计。

**这三条是同一个根因的三种表现，且这是 IA 方案 C 的正式反转**（方案 C 由用户此前选定，上手体验推翻了它 —— 这正是「先原型、再校准、后开发」这套方法要买的东西）。

| # | 决策 | 依据 |
|---|---|---|
| **D21** | **项目是空间，环节是时间。** 侧栏 = `驾驶舱 / 项目 / 游戏 / 洞察`（4 项）；环节只存在于项目空间内部，没有跨项目环节页 | 原侧栏把「项目」和「环节」并列，分组标签直接写着「环节工作台 · 跨项目」—— **结构本身在宣告「环节是你要去的地方，项目只是个过滤条件」**。但「05 内容交付」不是一个能去的地方，它是「gsea 现在走到哪了」。跨项目的需求由驾驶舱待办承担（它直达"某项目的某环节"）。 |
| **D22** | **一个环节只能有一套 UI。** `stagePanel` 是环节的唯一渲染入口 | 原本同一环节有两套：stagePanel（项目内摘要）+ renderXXX（跨项目实体），中间 `data-goto` 跳出去、`data-from` 锚点跳回来。**那个返回锚点就是设计缺陷的自白**：心流本就在项目内，何必跳出去再跳回来。`scopeBar` 里甚至有个按钮写着「在项目里打开 →」—— 环节页自己都知道它不是该待的地方。 |
| **D23** | **驾驶舱 = 跨项目（我该做什么）；项目空间 = 项目内（做）。每个角色的驾驶舱按其作业单元设计，不是同一套按权限过滤** | 原本三个角色共用一个 queue，BD 的驾驶舱里出现「知识库缺用研报告」（那是 01 Brief，营销负责人的环节）—— 这就是「工作流交叉」的具体形态。**权限过滤 ≠ 角色化设计。** 作业单元：Lead = 一个决策 / BD = 一段关系 / Finance = 一笔钱。 |
| **D24** | **财务的驾驶舱就是它的资金工作台，不给它造第七个环节** | 六环节 owner 是 `lead→bd→lead→bd→bd→lead`，**财务一个都不 own** —— 它唯一的动作 payout 是一个挂在别人环节里的按钮，动线变成「进项目→翻到 05→找结算面板」。可财务不关心项目走到哪，它关心钱，而钱是跨项目的。 |

**D25：环节工作台按 owner 的工作性质设计，非 owner 看到的是「结果」不是「选项」。**
用户 ③ 的落点。已实施的第一例：财务在 03 智能匹配看到的是「已定案组合 + 预算 + 承诺」，不是可点的方案选择卡 —— 给它一样的选择卡，就是把它拖进不属于它的决策里。
BD 在 04 的第一例：从「CRM 表 / 回复区 / 草稿列表」三块按数据类型分割（同一个创作者在三处各出现一次，BD 得自己在脑子里缝起来），改为按 **该谁动** 组织的关系推进面 —— 一个创作者 = 一行 = 一段关系，CRM 是它的状态而不是另一张表。
**其余 5 个环节（01/02/03/05/06）尚未按角色精细化**，等 BD 链路校准后展开。

**D20（工程纪律，非产品决策）：断言必须验行为，不得验源码关键字。**
复审 §6 点名 5 条假阳性断言，全部出自本实例之手 —— 最典型的是「交付审核通过后财务才可放款（**端到端**）」实际只检查源码里出现 `payoutReady` 这个词。**一个会撒谎的测试比没有测试更糟**：它用绿色买走了本该有的怀疑。
今后凡新增守卫 / 闸门 / 状态机类修复，须配**变异测试**：把修复退回原状，确认对应断言变红。未变红的断言视为无效断言。
推论：**功能被删除时，测它的断言必须一起删** —— 攻击面消失后留着的守卫断言会变成永远绿的空转（IA 收敛时据此删掉了 20 条测「环节页 × 作用域」的断言与 data-setscope 守卫用例）。
另一个真实教训：IA 重构时我用全局替换把 `delivery@gsea 不含 hsr 数据` 改成了 `goStage('hsr',5)`，**它一度在测自己名字的反面**。批量改测试后必须回头核对每条断言仍在测它名字所说的那件事。
