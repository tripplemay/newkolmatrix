...
- **D2 视觉等价口径**：除拍板改动（hover shadow-xl / gray-600 / 术语 / shadow 收敛 / campaigns:21 漂移修复）外，重构应视觉等价
  > **D2 拍板追认（2026-07-20 done 阶段 Planner 补记，来源 verify-B OBS-2）：** 以下变化属表面语言收敛的必然副产品，追认入拍板清单，与基线实测一致——TodoRow 边框 token 归一（gray-100→gray-200）、ExpertScope 由左条+shadow-sm 改为四边 border + 左条强调、边框几何引起的 ±2px 内容位移与一处摘要断行位移、ComingSoon 副标题间距 mt-2→mt-1、发送按钮 disabled 透明度 40→50
