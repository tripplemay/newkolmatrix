...
- **D-F tone 收敛 canonical 取 today 版**（dark 变体最全）。~~浅色基线零漂移~~ **Planner 修正（2026-07-22，fixing 轮）**：「浅色零漂移」前提对 campaigns 不成立——其 cr/wn 浅色类原本就与 today 版不同（`text-red-500`/`text-amber-700` vs `text-red-600`/`text-orange-600`），收敛后 campaigns 浅色 cr pill 实变 720px（Evaluator 像素取证）。定性为 **D-F 的意图变更**，按 D-I 重生 campaigns 基线并对账；wn 档差异当前无渲染面（四项目全 cr），M2/M3 三态丰富化后随基线自然显形，登记于此。canonical 裁决本身不变。
