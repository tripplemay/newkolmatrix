...
- **P10 列表上限 + 截断提示（Planner 批内裁决，2026-07-23）**：接真后库存 2500+ 行全渲染
  不可行——列表 followers 降序取前 `LIST_LIMIT=100`；KPI「库内创作者」恒显全量真值；
  截断时在表下方渲染一行次要文本提示（「按粉丝量显示前 100 位…」）——此行为**布局变更
  小注**（新元素，D2 数据诚实要求：不得让截断列表冒充全量），随 F004 落地并在
  ui-inventory 备注登记。原型无分页概念属 mock 时代 9 行样本的历史局限。
