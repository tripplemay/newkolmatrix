...
## 6. 复验（fix_round=1 · 隔离 fresh context · 2026-07-22）

- **复验人：** Andy/evaluator-subagent（reverifying 分片，与 §1-§5 上下文隔离）
- **复验对象：** HEAD `11d606c`（修复 commit `c362711` + linux 基线重生 `11d606c`）
- **环境：** standalone http://127.0.0.1:3000（构建对应 `5a666a9`；`git diff 5a666a9..HEAD` 实证产品代码零变更——仅 docs/状态文件/基线，构建对像素取证仍有效）；dev DB 复验前后均 pending=0 / oplog=0 / projects=4（全程只读零数据变更）
- **判定：** **PASS** —— 首轮唯一不成立子项「基线按 D-I 重生」已实证兑现，修复声明逐条证实

### 6.1 复验要点逐条实证

| # | 复验要点 | 判定 | 关键证据 |
|---|---|---|---|
| 1 | 两张 campaigns 基线重生晚于 F005 代码 commit | ✅ | `git log -- campaigns-darwin.png` 最后 = `c362711`；`campaigns-linux.png` 最后 = `11d606c`（github-actions bot，chore(visual) workflow 重生通道，campaigns-linux.png 单文件 Bin 266959→267023）；`git merge-base --is-ancestor` 实证 `2289343 → c362711 → 11d606c` 严格时序 |
| 2 | 当前渲染 vs 新 darwin 基线像素归零 | ✅ | 同口径复刻截图（1512×982 + mockFonts/mockHandoffs + fonts.ready + 1500ms settle + 动画禁用；脚本 `scripts/test/f005-reverify-pixel-diff.mjs`）：exact-diff **41px / maxΔ=1 / Δ>16=0**，bbox x1169-1495 y169-392（Copilot 侧栏亚感知级噪声，稳定化实现微差所致）；**pill 区 x531-963 零 diff**。断言余量恢复 1500−41=1459px |
| 3 | 新基线编码 red-600 真值 | ✅ | 新旧基线直接 diff = **恰 720px / maxΔ=51 / bbox x531-963 y250-461**（与首轮取证逐项吻合，无夹带变更）；色彩迁移 top6 全部浅红→深红反锯齿系；纯色统计（pill bbox 内）：旧基线 red-500 `(245,57,57)`=10 / red-600=0 → 新基线 red-500=0 / red-600 `(234,6,6)`=10——真值反转完备 |
| 4 | spec D-F 修正注记如实 | ✅ | spec :108 `~~浅色基线零漂移~~` strikethrough + Planner 修正注记（campaigns cr/wn 浅色类原本即与 today 版不同 / 720px 取证引用 / 定性 D-F 意图变更按 D-I 重生 / wn 档无渲染面登记 M2-M3 显形 / **canonical 裁决本身不变**）；`git show c362711` spec diff 恰 1 行替换，未触碰 §2 :79 原始裁决文本之外的任何裁决 |
| 5 | 视觉套件 campaigns 用例复跑 | ✅ | `BASE=:3000 npx playwright test -c tests/visual/playwright.evaluator.config.ts -g "campaigns list"` → **1 passed (2.0s)**（在重生后的新基线上绿，非借绿） |

### 6.2 检测器活性证明（v1.0.6 clause）

归零结论配 A/B 对照排除「探针失效假绿」：同一脚本同一截图对**旧基线**（`git show 7f86062` 提取）比对 → exact-diff **761px（=720 漂移 + 41 同源噪声）/ Δ>16=540 / maxΔ=51**，逐项复现首轮发现的量化指标。仪器在旧基线上仍能测出 720px 漂移、在新基线上归零——归零是基线修正的结果，不是测量失效。

### 6.3 结论

首轮 PARTIAL 的唯一缺陷（720px 意图变更借绿、基线未按 D-I 重生、D-F 登记失实）三项修复全部实证兑现：darwin 基线 `c362711` 精确重生（720px 恰为 pill 色变、零夹带）、linux 基线经既有 workflow 通道重生（`11d606c`）、spec D-F Planner 修正注记落盘且 canonical 裁决未被改写。首轮已 PASS 的 8 项 acceptance 涉及面产品代码零变更（`git diff 5a666a9..HEAD` 无 src/ 改动），无回归风险。

**F005 复验判定：PASS。**

> 复验产物：`scripts/test/f005-reverify-pixel-diff.mjs`（可复跑取证脚本，随本报告入库）；截图与旧基线副本存 `/tmp/f005-reverify/`（临时，不入库）。
