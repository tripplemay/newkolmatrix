...
### 4.2 长验收链的三条结构性规律（v1.0.15 — KOLMatrix M5.1c 六个隔离 agent 沉淀）

**① 对抗复核的产出不止「维持 / 推翻」，还包括纠正被复核报告本身。**
M5.1c 的对抗复核维持了两条 PARTIAL，同时对两份被复核报告查出 6 处瑕疵：把必修项从 2 条**扩到 3 条**
（验收方漏掉一条同族探针）、纠正被引行号、**下修**一处过重的严重性类比、指出「只测了 5 种里的第 1 种
就下结论」取证不足，并新查出一条两份报告都没提的计数失准。
**落地：对抗复核的 prompt 要显式要求它检查被复核报告的方法缺口**——版本集选得全不全、取证够不够、
严重性定性有没有偏——而不只是「结论对不对」。它最有价值的一次输出恰恰是「你这条发现**低估**了」。

**② fan-out 验收有一个结构性盲区：「全 PASS」这个前提本身没人核。**
每路只对自己射程内的条目负责，**合起来是否真的覆盖了全部 acceptance、有没有哪一条两路都没验却被
默认为通过**——没有角色天然负责这件事。M5.1c 的签收方把它当第一件事做：13 个 acceptance 条目逐条对表，
找出 3 个「只有首轮验过、复验未重放」的子条款，并把其中最实质的一条在被验 HEAD 上自己重做了一遍。
**落地：签收方的职责清单里显式加入「自行核实『全 PASS』前提成立」。**

**③ 变异要从「现实侧」打至少一次。**
M5.1c 前五路共打了 40+ 条变异，**全部**是改实现或改期望常量；没有一条从这道钉的**生产用途那一侧**打——
即「往 `src/` 里真的塞一个未登记的 importer，看它红不红」。签收方补了这一枪（三形态合一 → 两条用例红
并点名探针文件）。**规律：守「现实中会不会发生 X」的钉，必须有一条变异真的制造一次 X**；
...
| 2026-08-10 | §4.2 长验收链三条结构性规律：对抗复核须查被复核报告的方法缺口 · 「全 PASS」前提需签收方自核 · 变异须从现实侧打一次（v1.0.15） | KOLMatrix M5.1c-CENSUS-SCANNER |
