...
### 3.1 覆盖率 / 门禁类 acceptance：门限范围不得大于本批实际拥有的代码（v1.0.8 — KOLMatrix M1-A-BRIEF F001 沉淀）

起草含「覆盖率门」「静态门禁」「0 findings」类 acceptance 时，**门作用的范围（如 vitest `coverage.include`）必须限定在本批次实际负责的路径**，不能照抄一个远大于本批范围的理想范围。

否则会逼出两种坏结局，二者都坏：

| 配法 | 结局 |
|---|---|
| 照字面配大范围 + 接进 CI | 本批没写的既有未测代码把覆盖率拉到门限以下 → **CI 永久红**，批次无法完成 |
| 照字面配大范围 + 不接 CI | 门写在配置里却从不执行 → 退化成**「只有文案的门」**（PRD 反模式），验收时「有没有这条 script」看着绿、实则空 |

**反面（KOLMatrix M1-A-BRIEF F001）：** spec 照 architecture 规划写了 `coverage.include: ['src/lib/**'] + lines:80`，但 `src/lib` 下 agent/ · ai/ · mock/ 等既有代码本批不负责补测，整体实测行覆盖仅 6.2%。Generator 开工时撞见，走 pre-impl 审计裁决（D17）把 include 收窄到本批拥有的 `domain/** + data/provenance.ts`，门才既是真的、又接得进 CI。

**落地：**
- acceptance 写门限范围时，同步写一句「范围 = 本批新增/负责的路径」，并给出**后续放宽路径**：「随后续批次把新代码纳入，逐步放宽回理想范围；**不得反向下调 threshold** 来凑绿」。
- 别把「理想终态范围」和「本批门限范围」混为一谈——前者进 architecture 演进目标，后者进 acceptance。
