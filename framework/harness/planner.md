...
### 3.1 覆盖率 / 门禁类 acceptance：门限范围不得大于本批实际拥有的代码（KOLMatrix 本地沉淀）
...
| 照字面配大范围 + 不接 CI | 门写在配置里却从不执行，退化成只有文案的门 |

acceptance 应同时说明「范围 = 本批新增或负责的路径」及后续扩大范围的路径；不得反向下调 threshold 来凑绿。
