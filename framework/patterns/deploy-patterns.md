...
## 8. Compose 人工副本漂移：deploy workflow 不同步 compose 时，改 compose 的批次上线前必须显式同步（v1.0.10 — KOLMatrix M1-D F006 沉淀）

### 8.1 坑

`deploy-prod` 类 workflow 常见形态是「SSH 进 VPS → `compose pull` → `up -d`」——**compose 文件本身不在同步链上**，VPS 上的 `docker-compose.prod.yml` 是首次上线时的人工副本。此后任何批次修改了 repo 内 compose（新增卷 / env / service），若上线前忘记先把新 compose 拷到 VPS，deploy 会**静默跑旧配置**：

- 不报错、health 照样绿（旧 compose 本身是自洽的）
- 新增卷缺失时数据落容器层，**容器重建即丢**（静默持久化缺口）
- 新增 env 缺失时代码走默认值，行为漂移难察觉

### 8.2 规律

1. **凡批次改 `docker-compose.prod.yml`，deploy 前置第 0 步 = 同步 compose 到 VPS**（scp + 旧版 `.bak-<batch>` 备份），且该前置必须写进部署 runbook 的显式警示段（不能只活在 spec 里）
2. **Planner spec 起草 checklist**：feature 涉及 compose 改动时，acceptance 必含「deploy runbook 已加/已含 compose 同步前置」+ 上线验证含「`docker inspect` 实证新卷/env 已生效」
3. **Evaluator 部署后验证**：不只看 health——`docker inspect <app> --format '{{range .Mounts}}...'` 核卷挂载 + `docker exec <app> env | grep <新VAR>` 核 env 注入
4. **长期解法（候选）**：deploy workflow 加 checkout + scp compose 同步步——与 §5.1「deploy-script 与 yml 同 commit」同族（本条是它的镜像问题：那条是 script↔yml 漂移，这条是 compose↔VPS 副本漂移）

### 8.3 实操模板（M1-D 实操验证）

```bash
# 上线前置（改 compose 的批次）：
ssh <vps> 'cd /opt/apps/<app> && cp docker-compose.prod.yml docker-compose.prod.yml.bak-<batch>'
scp docker-compose.prod.yml <vps>:/opt/apps/<app>/docker-compose.prod.yml
ssh <vps> 'cd /opt/apps/<app> && grep -c "<新卷名>\|<新ENV>" docker-compose.prod.yml'  # 非 0 = 同步成功
# 然后才触发 deploy workflow；部署后：
ssh <vps> 'docker inspect <app容器> --format "{{range .Mounts}}{{.Name}} -> {{.Destination}}{{println}}{{end}}" && docker exec <app容器> env | grep <新VAR>'
```

**反面（未落但已近距离擦过）：** KOLMatrix M1-D F006 新增 `newkolmatrix-materials` 卷 + 2 个 env；探针漂移扫描时才发现 deploy-prod.yml 全程不碰 compose 文件——若直接触发部署，materials 卷缺失、上传文件落容器层且无任何报错。实操按 §8.3 模板同步后部署，`docker inspect` 实证卷挂载 + env 生效。

**来源：** KOLMatrix M1-D-KNOWLEDGE F006（2026-07-22，用户 Accept 沉淀 v1.0.10）。
...
| 2026-07-22 | §8 Compose 人工副本漂移：deploy workflow 不同步 compose 时改 compose 批次上线前必须显式同步（scp + .bak 备份 + docker inspect 实证）| KOLMatrix M1-D F006（v1.0.10）|
