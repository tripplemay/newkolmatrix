...
biz('RE-GAP-002 BD 超阈值组合（$9,800 > $8,000）→ 必须审批，未批不生效', () => {
  ...
  if ((canvas._html.match(/data-rel="/g) || []).length)
...
biz('角色化驾驶舱：BD 看不到营销负责人的待办', () => {
  if (global.__resetForTest) global.__resetForTest();
  switchTo('bd');
  navs[0].onclick();
  const h = canvas._html;
  if (/用研报告|知识覆盖度/.test(h))
    return 'BD 驾驶舱出现「知识库缺用研报告」—— 那是 01 Brief，营销负责人的环节';
  if (/复盘草案待采纳/.test(h))
    return 'BD 驾驶舱出现「复盘草案待采纳」—— 那是 06，营销负责人的环节';
  if (/待放款/.test(h)) return 'BD 驾驶舱出现「待放款」—— 那是财务的闸门';
  return true;
});

biz('角色化驾驶舱：营销负责人看不到 BD 的执行待办', () => {
  switchTo('lead');
  navs[0].onclick();
  const h = canvas._html;
  if (/key 未领取/.test(h)) return '营销负责人驾驶舱出现 key 待办 —— 那是 BD 的动作';
  if (/终稿待标记可发布/.test(h))
    return '营销负责人驾驶舱出现终稿审核 —— 那是 BD 在 05 的动作';
  return true;
});

biz('角色化驾驶舱：财务的待办里只有钱', () => {
  switchTo('finance');
  navs[0].onclick();
  // 只查待办队列 —— 「项目运转」面板里的进度描述（如绝区零的「候选池待确认」）
  // 是状态不是待办，财务需要知道项目走到哪了才能对账
  const i = canvas._html.indexOf('需要你处理');
  if (i < 0) return '财务驾驶舱没有待办面板';
  const q = canvas._html.slice(i);
  if (/回复待跟进|候选池待确认|key 未领取|终稿待标记/.test(q))
    return '财务的待办队列里出现了创作者运营的活 —— 它的作业单元是一笔钱';
  return true;
});

biz('BD 驾驶舱有「我的关系推进」（作业单元 = 一段关系，跨项目）', () => {
  if (global.__resetForTest) global.__resetForTest();
  switchTo('bd');
  goStage('gsea', 3);
  ...
  navs[0].onclick();
  const h = canvas._html;
  if (!/我的关系推进/.test(h)) return 'BD 驾驶舱没有关系推进概览';
  if (!/该谁动|等你动/.test(h)) return '关系概览没有按「该谁动」组织';
  return true;
});

biz('04 关系面按角色分化：财务在 03 看到的是结果不是选项', () => {
  switchTo('finance');
  canvas._html = '';
  click({ 'data-camp': 'gsea', 'data-stage-jump': '3' });
  if (/data-plan="/.test(canvas._html))
    return '财务拿到了可点的组合方案卡 —— 组合决策不是它的闸门（工作流交叉）';
  if (!/已定案组合|尚未定案/.test(canvas._html))
    return '财务在 03 看不到组合的预算结论 —— 它需要知道承诺了多少钱';
  return true;
});
...
biz('GAP-005 对象级权限在【所有入口】生效（驾驶舱/项目列表/详情直达）', () => {
  if (global.__resetForTest) global.__resetForTest();
  switchTo('bd');
  navs[0].onclick();
...
biz('GAP-004 批量发信 > 10 人转审批（真行为：提交→待批→批准→逐封生效）', () => {
  ...
  const pend = (canvas._html.match(/data-rel="/g) || []).length;
...
(() => {
  ...
  step('④ 发一封 → 信号链推进 CRM（sent→delivered→opened→replied）', () => {
    ...
    goStage('gsea', 4);
    // CRM 不再是一张孤立的表 —— 它是每一段关系行上的状态
    if (!/data-rel="/.test(canvas._html)) return '关系推进面没有渲染';
    if (!/已触达|已送达|已读|洽谈中/.test(canvas._html))
      return 'CRM 状态没有被信号推进';
    if (!/信号自动推导|信号推导/.test(canvas._html))
      return '关系行没有标注状态的推导方式';
    return true;
  });
