...
function StaticHandoffCard() {
  const from = personaBoundary(HANDOFF_FIXTURE.fromAgent)?.name ?? HANDOFF_FIXTURE.fromAgent;
  const to = personaBoundary(HANDOFF_FIXTURE.toAgent)?.name ?? HANDOFF_FIXTURE.toAgent;
  return (
      ...
      <div className="rounded-xl border border-gray-200 bg-white">
        ...
        <div className="border-t border-gray-100 px-3 py-2">
          ...
          <div className="mt-2 flex items-center gap-1 text-[11px] text-gray-500">
            ...
            交接物：{HANDOFF_FIXTURE.artifactType}（{HANDOFF_FIXTURE.artifactRef}）
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 py-8">
      <div className="mx-auto max-w-[420px]">
        <div className="mb-3">
          <div className="text-sm font-bold text-navy-700">Copilot · 多 Agent 编队</div>
          <div className="text-[11px] text-gray-400">
            hello-agent 端到端产物：自然语言 → search_kols → KOL 卡片流 → 一次协同交接
          </div>
        </div>
        ...
        <div className="mt-3 space-y-3">
          <div className="flex justify-end">
            <div className="max-w-[85%] rounded-2xl rounded-br-md bg-gradient-to-br from-brand-400 to-brand-600 px-3.5 py-2.5 text-[13px] leading-relaxed text-white">
              找《坦克世界》题材的游戏解说 KOL，给我候选
            </div>
          </div>
          <div className="flex justify-start">
            <div className="max-w-[90%] rounded-2xl rounded-bl-md bg-white px-3.5 py-2.5 text-[13px] leading-relaxed text-navy-700 shadow-sm">
