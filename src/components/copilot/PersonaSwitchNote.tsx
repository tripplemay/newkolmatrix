...
//
// 【M4.7 F003/F008 语义变更】单一前台之后，**对话身份恒为前台**，用户面对的始终是
// 同一个助手；专家的参与以 `ConsultationNote` 的协作痕迹呈现。
// 本组件只服务于**仍然存在的 handoff_to 路径**（M4.5 循环内接力，编排独占）——
// 它表示"这一轮里工作被交给了另一位专家"，而不再表示"面板换人了"。
// 两条路径并存且语义不同：consult = 问出去、答完即回；handoff = 交出去、后续步换人。
//
