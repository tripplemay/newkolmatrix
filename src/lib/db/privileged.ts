...
// >   「盲区登记」段登记了它抓不到的形态；
// > ② 这句话把两道钉当成了一道，而两个文件的头注都要求二者不得互相派生。
// > 承诺覆盖面的写法把读者的核查责任转移给了一道谁也没证明其完备的判据；改为只指路。
// >
// > fix-1 再更正（M5.1c 首轮验收 + 对抗复核，两方各自实测）：上一版的 ① 写的是
// > 「『盲区登记』段就列着**四种**作者未采用的写法**能绕过它**」——**把原文读反了**。
// > 那段说的是「用四种写法**打过**本钉」，而这四种**全部被抓到**；真正绕得过的是另一种
// > （标识符从不作为完整 token 出现的拼接键取用）。逐种实测：被抓到 4、能绕过 1，
// > 且拿其中第 1 种（`import * as ns` 后 `ns.privilegedDb`）真去打那道钉会被当场点名。
// > 故此处不再复述任何数量——数量正是上一版栽进去的地方，指路不需要它。
// >（同一族缺陷这次是**镜像方向**：不是把钉说强，是把钉说弱。机制一样：落笔时没跑那条命令。）
//
