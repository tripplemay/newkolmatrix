...
// **本注释不对那道钉的覆盖面作任何承诺** —— 它抓得到什么、抓不到什么，一律以那个文件的
// 头注与用例名为准（那里有变异证活的实跑记录，也有边界登记）。
//
// > 更正（M5.1c F002）：上一版这里把读者指向那个文件的**两个段落标题**
// > （「扫描口径与两类已知假阳性」「已知边界」）。同批 F001 把判据从正则换成 AST 时重写了
// > 文件头，前一个标题随之不存在 —— 指路句自己就漂了。段落标题和行号是同一类东西：
// > 最容易漂、且本段没有钉守它。改为只指到**文件**，段落由读者在那里自己找。
//
