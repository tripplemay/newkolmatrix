...
// F004/F005 带假（批末实测 15 处 / 11 文件），而当时挂在段尾那句「把核查责任转给
// db-layer-importer-census」的话**根本不成立**——该钉的 GUARDED 只有 runtime 与 privileged
// 两个 importer 条目，于是全仓 1869 条测试全绿、没有一条会红。
...
// 因此只写不随批次漂移的**设计性质**。
//
// 【这条规矩有一组钉在守：tests/unit/db-layer-importer-census.test.ts 的「说明段事实性陈述」】
// **本注释不对那组钉的覆盖面作任何承诺。** 它守什么、射程到哪、已知抓不到哪些形态，
// 一律以那个文件的用例名与头注为准（那里有变异证活的实跑记录，也有盲区登记）。
// 改本段之前请先读它——而不是读这里的转述。
//
// > 这一段本身栽过一次（fix-2 更正）：上一版写「三条已被实测证伪的绝对句不得复活」，而黑名单
// > 拍平成三个字符串后，其中两个同出首轮第 ③ 条，而第 ① 条（关于「单层」那句）实际无人守——
// > 逐字写回全仓 1875 条仍全绿。复验与对抗复核以「同一行位置换成别的黑名单串则当场红」的对照
// > 排除了「扫描器没看见」这一反向解释（docs/test-reports/M5.1b-rv1-F008-F001.md ·
// > M5.1b-adversarial-rv1-F008.md）。**同一个缺陷族在为消灭它而写的那句话里复发了一次**，
// > 故改成了 claim → patterns 的结构。
// >
// > fix-3 再更正：上一版接着写了「条数就是数据结构本身，声称与实际不可能再对不上」——
// > **这句又被证伪**：那条自洽断言只查条数与「每条至少有一个 pattern」，不查 pattern 有没有效；
// > 把 pattern 里的字换成简繁形近字，条数不变而守护面实际缩水，断言一声不吭
// >（docs/test-reports/M5.1b-rv2-F008-F001.md）。同一族缺陷第三次复发，都发生在「我为消灭它
// > 而写的那句话」里。用户据此裁决：**注释只指向钉的位置，不承诺完备性**。上面已照此改写。
