...
// 一组断言守着**本文件全文**（不只这段说明）——① 上面那条政策句不得被删；② 首轮验收逐条点名的
// 三条被证伪陈述一条都不得复活（条数由该文件的 FALSIFIED_CLAIMS 结构本身产出，另有一条断言钉住
// 「守的条数 = 首轮点名的条数」）；③ 不得再出现「N 处 withTenant 调用点」形态的计数。
// **该组断言配有变异证活实跑记录**（见那个文件的用例头注），不是又一句「已被守住」的空话。
//
// 【射程如实说清楚：本组**按句式匹配**，改写措辞可绕过】同义改写、中文数字、「个」替「处」、
// 语序颠倒都不被抓。写在这里是因为——本组断言存在的全部意义就是不让「声称的覆盖面」超过
// 「实际覆盖面」，那它自己更不能犯。
//
