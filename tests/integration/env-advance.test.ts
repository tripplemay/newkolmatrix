...
describe('守卫拒绝 → 零写入（失败尝试不得掺进北极星指标）', () => {
  ...
  it('前置未满足的三条被拒，各自零写入且游标不动（M3-B F010：三条全为真判定理由）', async () => {
    // M2-A F004：match→reach = MATCH_PLAN_NOT_APPROVED（夹具项目无 approved MatchPlan）
    // M3-B F010：reach→delivery = NO_DEAL_YET（夹具项目无 Deal）；
    //            delivery→insight = DEALS_NOT_SETTLED？——零 Deal 时 allDealsSettled=true
    //            即放行（P12 空态诚实），故此列改在下方独立用例断言放行语义。
    const denied: Array<
    ...
    for (const [from, expectedReason] of denied) {
  ...
  it('P12 空态诚实：零 Deal 项目 delivery→insight 放行（不被交付条件卡住）', async () => {
    await resetProject('delivery', 'delivery', GOAL);
    const r = await advanceStage({ projectId, tenantId });
    expect(r.ok).toBe(true);
