...
describe('zod 契约（真样本 pin，v0.9.19）', () => {
  ...
  it('已知 nullable 形态被容忍：YT qualityScore null / IG businessCategory 空串 / following null', () => {
    ...
    expect(apifyKolRowSchema.safeParse({ ...yt, following: null }).success).toBe(true);
    expect(apifyKolRowSchema.safeParse({ ...ig, businessCategory: '' }).success).toBe(true);
    expect(apifyKolRowSchema.safeParse({ ...yt, matchedTags: [] }).success).toBe(true);
  });
...
describe('listKols client 行为（【P7】fetch 注入）', () => {
  ...
  it('【P2】URL 不带 platform 过滤，分页参数正确 + x-api-key 经 headers（形状经真信封）', async () => {
    let seenUrl = '';
