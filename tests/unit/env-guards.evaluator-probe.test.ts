...
// M3-B F010 机械同步：扩 NO_DEAL_YET / DEALS_NOT_SETTLED（→delivery / →insight 真判定），
// D9 占位理由 DEPENDENCY_NOT_IMPLEMENTED 随之退役（五条流转全部真判，无分支再返回它）。
...
/** 各 cur 在「判据全 false」下的真判定拒绝理由（M2-A F004 + M3-B F010）。 */
const EXPECTED_DENY: Record<string, string> = {
...
const ctx = (o: Partial<EnvGuardContext> = {}): EnvGuardContext => ({
  ...
  // M3-B F010 机械同步：再扩必填 hasDeal / allDealsSettled，默认 false（无 Deal、未收尾）——
  // 「未满足前置一律不放行」这条探针本意不变，只是 reach/delivery 两列的理由翻为
  // NO_DEAL_YET / DEALS_NOT_SETTLED。
...
describe('[Evaluator] 未满足前置的流转在全合法状态空间不放行（不只 cur===maxReached 那一列）', () => {
  it('cur ∈ {match, reach, delivery} 且无已批准组合时，无论 maxReached 已解锁到多远、goal 是否齐备，一律不放行', () => {
    ...
    for (const { cur, maxReached } of VALID_STATES) {
      ...
      for (const goal of [GOAL, null]) {
        ...
        // 三列均为真判定理由（M2-A F004 + M3-B F010）
        expect(r.reason).toBe(EXPECTED_DENY[cur]);
      }
    }
  });
  ...
  it('[F004 扩] match→reach 仅在 hasApprovedMatchPlan=true 时放行；该判据不旁路 F010 两条', () => {
    expect(
      canAdvance(
