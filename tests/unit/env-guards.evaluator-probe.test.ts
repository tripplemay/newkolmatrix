...
const VALID_STATES: Array<{ cur: Stage; maxReached: Stage }> = STAGES.flatMap(
  (cur, ci) =>
    STAGES.filter((_, mi) => mi >= ci).map((maxReached) => ({ cur, maxReached })),
);
...
describe('[Evaluator] D9 保守拒绝在全合法状态空间成立（不只 cur===maxReached 那一列）', () => {
  it('cur ∈ {match, reach, delivery} 时，无论 maxReached 已解锁到多远、goal 是否齐备，一律不放行', () => {
    const d9Cur: Stage[] = ['match', 'reach', 'delivery'];
    for (const { cur, maxReached } of VALID_STATES) {
      if (!d9Cur.includes(cur)) continue;
      for (const goal of [GOAL, null]) {
        const r = canAdvance(ctx({ cur, maxReached, goal }));
        expect(r.allowed, `cur=${cur} maxReached=${maxReached} goal=${goal ? 'set' : 'null'}`).toBe(false);
        expect(r.reason).toBe('DEPENDENCY_NOT_IMPLEMENTED');
      }
    }
  });

  it('即便 maxReached 已到末环节（历史上曾解锁过），D9 三条仍不放行 —— 解锁历史不得成为绕过依赖检查的后门', () => {
...
describe('[Evaluator] D2 回退态（cur < maxReached）—— 双值设计的唯一理由，须行为明确', () => {
  it('回退到 brief 后，goal 齐备则可重新推进；goal 被清空则拒绝（判据仍生效，不因曾解锁而豁免）', () => {
    expect(canAdvance(ctx({ cur: 'brief', maxReached: 'insight', goal: GOAL })).allowed).toBe(true);
    expect(canAdvance(ctx({ cur: 'brief', maxReached: 'insight', goal: null }))).toEqual({
      allowed: false,
  ...
  it('回退态下 canEnter 的解锁范围仍以 maxReached 为准，不随 cur 收缩', () => {
    ...
    STAGES.forEach((target, t) => {
      expect(canEnter(back, target).allowed, `target=${target}`).toBe(t <= stageIndex('delivery'));
    });
  });
...
describe('[Evaluator] D8 返回结构在全输入空间恒定（无 undefined / 无自由文本 / 不抛错）', () => {
  const WEIRD = ['', 'nope', 'BRIEF', 'brief ', '0', '__proto__'] as unknown as Stage[];
    ...
    for (const cur of ALL) {
      for (const maxReached of ALL) {
        for (const target of ALL) {
          ...
          expect(typeof r?.allowed, `${cur}/${maxReached}/${target}`).toBe('boolean');
          if (r.allowed) expect(r.reason).toBeNull();
          else expect(REASONS.has(r.reason as string), `理由越界: ${r.reason}`).toBe(true);
        }
      }
    }
  });
  ...
  it('非法环节取值一律被不变量拦下，绝不放行', () => {
    for (const bad of WEIRD) {
      ...
      expect(canEnter(ctx({ cur: bad, maxReached: 'insight' }), 'brief').allowed).toBe(false);
      expect(canAdvance(ctx({ cur: bad, maxReached: 'insight' })).allowed).toBe(false);
    }
  });
  ...
  it('合法组合：raise 结果序号 >= 原 maxReached 序号', () => {
    for (const m of STAGES) {
      for (const t of STAGES) {
        expect(stageIndex(raiseMaxReached(m, t))).toBeGreaterThanOrEqual(stageIndex(m));
      }
    }
  });
