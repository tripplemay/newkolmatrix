...
describe('kol-deep zod 契约（读宽松/写严格 + refine 给分必给依据）', () => {
  it('credibility：signals 空数组 = 非法（FR-11.4）；非空合法', () => {
    ...
    expect(credibilitySchema.safeParse({ ...base, signals: [] }).success).toBe(
      false,
    );
    expect(
  ...
  it('brandSafety：flags 空 = 非法；rating 枚举外拒', () => {
    ...
    expect(brandSafetySchema.safeParse({ ...base, flags: [] }).success).toBe(
      false,
    );
    expect(
  ...
  it('audienceDemo：interests 写侧非空；分布三键 optional；占比域 0-100', () => {
    expect(audienceDemoSchema.safeParse({ interests: ['fps'] }).success).toBe(
      true,
    );
...
describe('deriveCredibility（弱信号规则合成）', () => {
  ...
  it('权重和为 1（五因子全集，导出常量可测）', () => {
    const sum = Object.values(CREDIBILITY_WEIGHTS).reduce((s, w) => s + w, 0);
    expect(Object.keys(CREDIBILITY_WEIGHTS).sort()).toEqual([
      'businessEmail',
      'followers',
      'quality',
      'tier',
  ...
  it('fix_round 1 回归 pin：followers-only 行派生非 null（首轮验收探针案例）', () => {
    const c = deriveCredibility(row({ followers: 9_240_000 }), AT);
    expect(c).not.toBeNull();
    // log10(9.24e6+1)/7 ≈ 0.9951；单因子重归一 → value 直通
    expect(c!.score).toBe(
      Math.round(Math.min(1, Math.log10(9_240_001) / FOLLOWERS_LOG_CAP) * 100),
    );
    expect(c!.signals).toEqual(['粉丝规模 9,240,000']);
  });

  it('五因子齐备：含商务邮箱信号；hasBusinessEmail=false 计 0 但 signal 仍在（诚实）', () => {
    ...
    // 0.3·1 + 0.3·0.8 + 0.25·1 + 0.1·1 + 0.05·1 = 0.94 → 94（全员在场无重归一）
    expect(c!.score).toBe(94);
    expect(c!.signals).toHaveLength(5);
    expect(c!.signals).toContain('商务邮箱信号 ✓');
    const noEmail = deriveCredibility(
      row({ verified: true, hasBusinessEmail: false }),
      AT,
    );
    expect(noEmail!.signals).toContain('商务邮箱信号 ✗');
  });
  ...
  it('qualityScore 越界钳制 [0,1]；verified=false 计 0 但 signal 仍在（诚实）', () => {
    const c = deriveCredibility(
      row({ verified: false, qualityScore: 1.7 }),
      AT,
    );
